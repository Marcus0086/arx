...
pub mod upload_frame {
    ...
    pub enum Payload {
        #[prost(message, tag = "1")]
        PackHeader(super::PackHeader),
        #[prost(message, tag = "2")]
        CrudHeader(super::CrudHeader),
        #[prost(message, tag = "3")]
...
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct CrudDiffResponse {
    #[prost(message, repeated, tag = "1")]
    pub entries: Vec<CrudDiffEntry>,
}
...
pub trait ArxService: Send + Sync + 'static {
    async fn pack_stream(
        &self,
        request: tonic::Request<tonic::Streaming<UploadFrame>>,
    ...
    type ExtractStreamStream: futures_core::Stream<Item = std::result::Result<DownloadFrame, tonic::Status>>
        + Send
        + 'static;
    ...
    async fn issue(
        &self,
        request: tonic::Request<IssueRequest>,
    ...
    async fn whoami(
        &self,
        request: tonic::Request<WhoamiRequest>,
    ...
    B::Error: Into<tonic::codegen::StdError> + Send + 'static,
{
    ...
    fn call(&mut self, req: http::Request<B>) -> Self::Future {
        ...
        macro_rules! unary_handler {
            ($req_ty:ty, $resp_ty:ty, $method:ident) => {{
                ...
                let fut = async move {
                    ...
                    if let Some(sz) = max_dec {
                        grpc = grpc.max_decoding_message_size(sz);
                    }
                    if let Some(sz) = max_enc {
                        grpc = grpc.max_encoding_message_size(sz);
                    }
        ...
        macro_rules! client_streaming_handler {
            ($req_ty:ty, $resp_ty:ty, $method:ident) => {{
                ...
                impl<T: ArxService> tonic::server::ClientStreamingService<$req_ty> for Svc<T> {
                    ...
                    fn call(
                        &mut self,
                        req: tonic::Request<tonic::Streaming<$req_ty>>,
                    ) -> Self::Future {
                        let i = self.0.clone();
                        Box::pin(async move { i.$method(req).await })
                    }
        ...
        match req.uri().path() {
            "/arx.ArxService/PackStream" => {
                client_streaming_handler!(UploadFrame, PackResponse, pack_stream)
            }
            "/arx.ArxService/CrudAddStream" => {
                client_streaming_handler!(UploadFrame, CrudAddResponse, crud_add_stream)
            }
            ...
            "/arx.ArxService/CrudSync" => {
                unary_handler!(CrudSyncRequest, CrudSyncResponse, crud_sync)
            }
            "/arx.ArxService/CrudDiff" => {
                unary_handler!(CrudDiffRequest, CrudDiffResponse, crud_diff)
            }
            "/arx.ArxService/ChunkMap" => {
                unary_handler!(ChunkMapRequest, ChunkMapResponse, chunk_map)
            }
            "/arx.ArxService/ListArchives" => {
                unary_handler!(ListArchivesReq, ListArchivesResp, list_archives)
            }
            ...
            "/arx.ArxService/RefreshToken" => {
                unary_handler!(RefreshTokenRequest, RefreshTokenResponse, refresh_token)
            }
            ...
            "/arx.ArxService/CreateTenant" => {
                unary_handler!(CreateTenantRequest, CreateTenantResponse, create_tenant)
            }
            "/arx.ArxService/CreateUser" => {
                unary_handler!(CreateUserRequest, CreateUserResponse, create_user)
            }
            "/arx.ArxService/CreateApiKey" => {
                unary_handler!(CreateApiKeyRequest, CreateApiKeyResponse, create_api_key)
            }
            "/arx.ArxService/RevokeApiKey" => {
                unary_handler!(RevokeApiKeyRequest, RevokeApiKeyResponse, revoke_api_key)
            }
            "/arx.ArxService/ListTenants" => {
                unary_handler!(ListTenantsRequest, ListTenantsResponse, list_tenants)
            }
            "/arx.ArxService/ListUsers" => {
                unary_handler!(ListUsersRequest, ListUsersResponse, list_users)
            }
