...
pub struct ChunkParams {
    /// Minimum chunk size (must be >= 64)
    pub min: usize,
    /// Target/average size (power-of-two-ish, e.g. 256 KiB)
    pub avg: usize,
    /// Maximum chunk size (must be > avg)
    pub max: usize,
}

impl Default for ChunkParams {
    fn default() -> Self {
        // Reasonable defaults for general data; tweak in packer.
        Self {
            min: 64 * 1024,       // 64 KiB
            avg: 256 * 1024,      // 256 KiB
            max: 1 * 1024 * 1024, // 1 MiB
        }
    }
}

/// Streaming CDC chunker using a Gear rolling hash.
/// Keeps a small stash buffer to avoid over-reading beyond a boundary.
pub struct StreamingChunker {
    p: ChunkParams,
    mask: u64,
    // Rolling fingerprint
    fp: u64,
    // Internal read buffer & cursor
    stash: Vec<u8>,
    pos: usize,
    // Scratch read buffer to refill stash from the reader
    scratch: Vec<u8>,
}

impl StreamingChunker {
    pub fn new(p: ChunkParams) -> Self {
        assert!(p.min >= 64, "min too small");
        assert!(p.max > p.avg && p.avg >= p.min, "min <= avg < max required");
        let k = (usize::BITS - (p.avg.saturating_sub(1)).leading_zeros()) as u32 - 1;
        let mask = (1u64 << k).saturating_sub(1);
        Self {
            p,
            mask,
            fp: 0,
            stash: Vec::with_capacity(p.max * 2), // enough to hold largest chunk
            pos: 0,
            scratch: vec![0u8; 64 * 1024],
        }
    }

    /// Read the next chunk into `out`. Returns number of bytes (0 at EOF).
    pub fn next_chunk<R: Read>(&mut self, r: &mut R, out: &mut Vec<u8>) -> Result<usize> {
        out.clear();

        // If stash is empty (or fully consumed), refill it.
        if self.pos >= self.stash.len() {
            self.stash.clear();
            self.pos = 0;
            let n = r.read(&mut self.scratch)?;
            if n == 0 {
                return Ok(0); // EOF and no pending bytes
            }
            self.stash.extend_from_slice(&self.scratch[..n]);
        }

        self.fp = 0;
        let mut size = 0usize;

        // Consume bytes from stash, refilling as needed, until we hit a boundary.
        loop {
            // If we exhausted stash, pull more from the reader.
            if self.pos >= self.stash.len() {
                self.stash.clear();
                self.pos = 0;
                let n = r.read(&mut self.scratch)?;
                if n == 0 {
                    // EOF: flush what we have (even if < min)
                    break;
                }
                self.stash.extend_from_slice(&self.scratch[..n]);
            }

            // Take one byte, advance cursors.
            let b = self.stash[self.pos];
            self.pos += 1;
            out.push(b);
            size += 1;

            // Rolling gear: fp = (fp << 1) + G[b]
            self.fp = (self.fp << 1).wrapping_add(GEAR[b as usize]);

            // Enforce maximum size hard stop
            if size >= self.p.max {
                break;
            }
            // Allow boundary only if we've reached the minimum
            if size >= self.p.min {
                // Cut if pattern matches
                if (self.fp & self.mask) == 0 {
                    break;
                }
            }

            // Continue otherwise; loop pulls more bytes as needed
        }
...
// Build-time initializer to fill the full table if we provided a short seed above.
// If you want, you can replace the entire table with a canonical 256-entry constant.
#[allow(dead_code)]
const fn build_full_gear() -> [u64; 256] {
    let mut t = [0u64; 256];
    // copy the seeded ones we wrote by hand:
    let seed_len = 32;
    let mut i = 0;
    while i < seed_len {
        t[i] = GEAR[i];
    ...
    // deterministically derive the rest
    let mut s = 0xC0FFEE_F00DBABE_u64;
    i = seed_len;
    while i < 256 {
        s = prng64(s ^ (i as u64).wrapping_mul(0x9E37));
        t[i] = s;
        i += 1;
    }
    t
